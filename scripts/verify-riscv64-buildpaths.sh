...
# `packages/os/toolchains/cmake/toolchain-riscv64-linux-musl.cmake`. Inspects every produced
...
build_package() {
    ...
    if [ -n "$extra_flag" ]; then
        cmake -S "$pkgdir" -B "$builddir" \
            -DCMAKE_TOOLCHAIN_FILE="$repo_root/packages/os/toolchains/cmake/toolchain-riscv64-linux-musl.cmake" \
...
# ── Report ────────────────────────────────────────────────────────────
{
    ...
    echo "- Toolchain: \`packages/os/toolchains/cmake/toolchain-riscv64-linux-musl.cmake\`"
