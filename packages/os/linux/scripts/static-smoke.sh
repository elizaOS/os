...
REPO_ROOT="$(cd "${ELIZAOS_ELIZA_ROOT:-${ROOT}/../../../.eliza-source}" 2>/dev/null && pwd || true)"
if [ -z "${REPO_ROOT}" ] || [ ! -d "${REPO_ROOT}/packages/agent" ]; then
    echo "static-smoke: set ELIZAOS_ELIZA_ROOT to an elizaOS/eliza checkout." >&2
    exit 2
fi
...
bash -n build.sh build-iso.sh tails/auto/build \
    ...
    scripts/verify-eliza-source-lock.test.sh \
...
ELIZAOS_ELIZA_ROOT="${REPO_ROOT}" node scripts/verify-eliza-source-lock.mjs
