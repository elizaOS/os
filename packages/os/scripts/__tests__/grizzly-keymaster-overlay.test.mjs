...
test("keymaster diagnostic is reversible and never changes pinned init source", () => {
  ...
  try {
    ...
    const original = "on post-fs-data\n    exec - system system -- /system/bin/vdc keymaster earlyBootEnded\n";
    ...
    assert.equal((readFileSync(makefile, "utf8").match(/PRODUCT_COPY_FILES/g) ?? []).length, 1);
    normalizeAospKeymasterInit(root, false);
    assert.doesNotMatch(readFileSync(makefile, "utf8"), /diagnostic|PRODUCT_COPY_FILES/);
    assert.equal(existsSync(join(generated, "diagnostics/system/etc/init/hw/init.rc")), false);
    assert.equal(readFileSync(source, "utf8"), original);
