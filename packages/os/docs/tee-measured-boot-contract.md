...
`scripts/tee-evidence-bridge.mjs` produces this document. On real
...
fixtures (`release/schema/tee-evidence.mock.json` and
...
> `node scripts/tee-evidence-bridge.mjs --quote-source tappd --socket /var/run/dstack.sock`.

## dstack Hardening Pins

Before dstack is trusted in a high-assurance stack it must be pinned/hardened per
`linux/confidential/dstack-pins.json` (the data form of the plan
