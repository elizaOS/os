...
distribution, `linux`) and **elizaOS Android** (the AOSP fork,
`android`) — and where the fork boundary deliberately is.
...
the home surface (`linux/README.md`). GNOME Shell already ships a
...
manifest (`android/vendor/eliza/manifests/aosp-assistant-full-control.json`)
