...
- `docs/tee-measured-boot-contract.md` — the provider-neutral
  measurement set and key-release rules.
- `docs/tee-protected-agent-vm.md` — the macOS-feasible bridge and
  host↔guest contract.
...
The active Linux build (`linux/`) is the canonical Tails-derived
...
1. Vendors `meta-dstack` + the upstream layers it composes (`poky`,
   ...
   under `linux/confidential/`, pinned by commit + a `repro-build`
...
1. **Build time (reproducible, offline-verifiable):**
   `scripts/generate-tee-measurements.mjs` already hashes the
...
- **sepolicy gating** of the protected-agent binder/vsock: a dedicated
  ...
  `android/vendor/eliza/sepolicy/eliza_agent.te`) is the only domain
...
| OS-1 | `meta-elizaos` Yocto layer + `ELIZAOS_PROFILE=confidential` scaffold (vendored meta-dstack pin, repro-build context). No real image yet. | `linux/confidential/` skeleton + image-manifest schema (shared with chip WI-3) | 1.0 | `confidential-image-manifest-check` |
